...
This GitHub Action publishes the Helm chart from the given `directory` to an OCI registry
as an OCI artifact, the registry can be set with the `registry` input (default to quay.io).
The provided `username` and `token` should together be able to authenticate with the given
registry.
...
`quay.io/azimuth-cloud/<repository-name>/<chart-name>:<chart-version>`.
